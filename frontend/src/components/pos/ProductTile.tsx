...
export function ProductTile({ product, stockQuantity, onAdd }: { product: Product; stockQuantity?: number | null; onAdd: (product: Product) => void }) {
  ...
  const displayStock = stockQuantity ?? primaryVariant?.stockQuantity ?? 0;
  const displayPrice = primaryVariant?.discountPrice ?? primaryVariant?.sellingPrice ?? product.sellingPrice;
  const displayCode = primaryVariant?.barcode ?? primaryVariant?.sku ?? product.barcode ?? "بدون باركود";
    ...
    >
      <div className="mb-3 flex items-start justify-between gap-2">
        ...
        <div className="flex flex-col items-end gap-1">
          <span className="rounded-full bg-muted px-2 py-1 text-[11px] font-semibold text-muted-foreground">{primaryVariant ? `${primaryVariant.size} / ${primaryVariant.color}` : product.unitType}</span>
          <span className={`rounded-full px-2 py-1 text-[11px] font-semibold ${isAvailable ? "bg-success/10 text-success" : "bg-danger/10 text-danger"}`}>
            {isAvailable ? `متاح ${displayStock.toLocaleString("ar-EG")}` : "غير متوفر"}
          </span>
        </div>
      </div>
      <span className="line-clamp-2 font-bold text-foreground">{product.name}</span>
      <span className="mt-1 text-xs text-muted-foreground">{displayCode}</span>
      <div className="mt-auto flex items-end justify-between gap-2 pt-4">
        <span className="text-lg font-extrabold text-foreground">{displayPrice.toLocaleString("ar-EG")} ج</span>
